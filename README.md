...
| [silo-plugin-metadata-hardcover](https://github.com/totza2010/silo-plugin-metadata-hardcover) | totza2010 | No description provided. | ⚪ *Unverified* | `Silo SDK (gRPC)` | ❓ No |
...
| [silo-plugin-sync-watcharr](https://github.com/mildman1848/silo-plugin-sync-watcharr) | mildman1848 | Silo watch sync provider for two-way watched movie and episode sync with Watcharr | ⚪ *Unverified* | `Silo SDK (gRPC)` | ❓ No |
...
| [bloem-community-ironicbadger-tailscale](https://github.com/Bloem-Studios/bloem-community-ironicbadger-tailscale) | Bloem-Studios | Tailscale plugin for Silo Media Server | ⚪ *Unverified* | `Silo SDK (gRPC)` | ❓ No |
| [clawcor](https://github.com/Synxneuos/clawcor) | Synxneuos | We built clawcor 100% open-source because we believe the future of AI engineering belongs to the open developer community, not closed silos.  We are looking for developers, contributors, and builders to take clawcor to the next level (adding multi-modal terminal vision, local LLM optimizations, and custom agent plugins). | ⚪ *Unverified* | `Silo SDK (gRPC)` | ❓ No |
...
| [scholion](https://github.com/CrossRead/scholion) | CrossRead | Your genome, labs, prescriptions and wearables sit in silos that never compare notes, so contradictions go unnoticed. This reads them against each other, with the source behind every answer — one local engine behind a web UI, a CLI, a Claude skill, and an Ouroboros plugin. | ⚪ *Unverified* | `Silo SDK (gRPC)` | ❓ No |
...
| [silo-auto-translate](https://github.com/randrini/silo-auto-translate) | randrini | Silo plugin: auto extract+translate subtitles (FR) from rating webhooks via SubExtractor | ⚪ *Unverified* | `Silo SDK (gRPC)` | ❓ No |
| [silo-comic-pages](https://github.com/crowquillx/silo-comic-pages) | crowquillx | Silo plugin that extracts and caches CBR/CBZ comic pages for Aidoku | ⚪ *Unverified* | `Silo SDK (gRPC)` | ❓ No |
